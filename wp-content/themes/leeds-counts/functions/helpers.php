...
add_filter( 'post_thumbnail_html', '_3tone_remove_width_height_attributes', 21, 8 );
 ...
 * -----------------------------------------------*/
function _3tone_the_icon($icon_name, $title = true, $classes = '', $aria_hidden = false, $role = false){
  echo _3tone_get_icon($icon_name, $title, $classes, $aria_hidden, $role);
}

function _3tone_get_icon($icon_name, $title = true, $classes = '', $aria_hidden = false, $role = false){
  ...
  $svg .= '<svg class="icon icon--'. $icon_name .' '. $classes .'"'. ($aria_hidden ? ' aria-hidden="true"' : '') .''. ($role ? ' role="img"' : '') .'>';
...
function get_breakpoint_directions($breakpoints){
  ...
  foreach ($breakpoints as $k => $bp):
    ...
    if($end != null && $start != null):
      $down_key = $k .'-';
      $_bps = array_merge($_bps, [$down_key => [
        'start' => null,
        'end' => $end
      ]]);
