...
function get_image_url($img_id = false, $options = array()){
  if(!$img_id){ $img_id = get_post_thumbnail_id(); }
  if(!$img_id){ write_log('No image found / No ID provided'); return false; }

  // If the imgix plugin is installed
  if (function_exists ( 'replace_host' ) && ((is_admin() && defined( 'DOING_AJAX' ) && DOING_AJAX) || (!is_admin() && !defined('DOING_AJAX')))) {
    // Always add auto format
    $options['auto'] = 'format,compress';
    if(!isset($options['q'])) $options['q'] = 80;
...
function _3t_get_image_tag($img_id, $options = array(), $classes = ''){
  if(!$img_id || $img_id == ''){ $img_id = get_post_thumbnail_id(); }
  if(!$img_id || $img_id == ''){ write_log('No image found / No ID provided'); return false; }

  $img_url = get_image_url($img_id, $options);
  $img_meta = wp_get_attachment_metadata($img_id)['image_meta'];
  $img_tag = '<img src="'. $img_url .'" alt="'. $img_meta['caption'] .'" '. ($classes ? 'class="'. $classes .'"' : '') .' />';
...
function get_interchange_image_tag($img_id, $options = array(), $classes = ''){
  ...
  $img_meta = wp_get_attachment_metadata($img_id)['image_meta'];
  $img_tag = '<img '.get_interchange_data_attribute($img_id, $options).' alt="'. $img_meta['caption'] .'" '. ($classes ? 'class="'. $classes .'"' : '') .' >';
...
function get_interchange_data_attribute($img_id, $options = array()){
    return("data-interchange=\"".get_interchange_data_attribute_contents($image_id, $options).'"');
}
...
function get_media_base_url(){
  ...
  if(isset($as3cf)):
    ...
    $domain_bucket = $scheme .'://'. $as3cf->get_s3_url_domain( $bucket, $region) .'/';
