...
if (!class_exists('acf_admin_settings_updates')):
  class acf_admin_settings_updates
  {
    ...
    function show_notice($message = '', $class = '')
    {
      ...
      if (acf_has_done('acf_admin_settings_updates_notice')) {
        return false;
      }
    ...
    function show_error($error = '')
    {
      ...
      if (is_wp_error($error)) {
        $error =
          __('<b>Error</b>. Could not connect to update server', 'acf') .
    ...
    function load()
    {
      ...
      if (version_compare($info['version'], $version, '>')) {
        ...
        $this->view['changelog'] = $this->get_changelog_section(
          $info['changelog'],
          $info['version']
        );
        $this->view['upgrade_notice'] = $this->get_changelog_section(
          $info['upgrade_notice'],
          $info['version']
        );
