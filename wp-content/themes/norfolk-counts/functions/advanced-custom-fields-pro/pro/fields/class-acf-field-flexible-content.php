  ...
  class acf_field_flexible_content extends acf_field
  {
    ...
    function initialize()
    {
      ...
      $this->l10n = array(
        ...
        'max_layout' => __(
          "Maximum {label} limit reached ({max} {identifier})",
          'acf'
        ),
        'available' => __(
      ...
      // filters
      $this->add_filter('acf/prepare_field_for_export', array(
        $this,
        'prepare_any_field_for_export'
      ));
      $this->add_filter(
        'acf/clone_field',
        array($this, 'clone_any_field'),
        10,
        2
      );
      $this->add_filter('acf/validate_field', array(
        $this,
        'validate_any_field'
      ));
      ...
      $this->add_field_filter(
        'acf/get_sub_field',
        array($this, 'get_sub_field'),
        10,
        3
      );
      $this->add_field_filter('acf/prepare_field_for_export', array(
        $this,
        'prepare_field_for_export'
      ));
      $this->add_field_filter('acf/prepare_field_for_import', array(
        $this,
    ...
    function get_sub_field($sub_field, $selector, $field)
    {
      ...
      foreach ($field['layouts'] as $layout) {
        ...
        foreach ($layout['sub_fields'] as $sub_field) {
          // check name and key
          if (
            $sub_field['name'] == $selector ||
            $sub_field['key'] == $selector
          ) {
            // return
            return $sub_field;
          }
...
<div <?php acf_esc_attr_e($div); ?>>
   ...
   echo 'style="display:none;"';
	...
	<script type="text-html" class="tmpl-popup"><?php  ?><div class="acf-fc-popup"><ul><?php foreach (
  $layouts
  as $layout
):
  $atts = array(
...
<div <?php echo acf_esc_attr($div); ?>>
   ...
   'acf'
    ...
    'acf'
		...
		<a class="acf-icon -collapse small acf-js-tooltip" href="#" data-name="collapse-layout" title="<?php _e(
    'Click to toggle',
    'acf'
  ); ?>"></a>
	</div>
  ...
  'key'
	...
	<td class="acf-label">
		...
		<p class="description acf-fl-actions">
			<a data-name="acf-fc-reorder" title="<?php _e(
     "Reorder Layout",
     'acf'
   ); ?>" ><?php _e("Reorder", 'acf'); ?></a>
			<a data-name="acf-fc-delete" title="<?php _e(
     "Delete Layout",
     'acf'
   ); ?>" href="#"><?php _e("Delete", 'acf'); ?></a>
			<a data-name="acf-fc-duplicate" title="<?php _e(
     "Duplicate Layout",
     'acf'
   ); ?>" href="#"><?php _e("Duplicate", 'acf'); ?></a>
			<a data-name="acf-fc-add" title="<?php _e(
     "Add New Layout",
     'acf'
...
<?php
    ...
    function validate_value($valid, $value, $field, $input)
    {
      ...
      if ($min && $min > $count) {
        ...
        $error = $this->l10n['min'];
        $identifier =
          $min == 1 ? $this->l10n['layout'] : $this->l10n['layouts'];
    ...
    function get_layout_title($field, $layout, $i, $value)
    {
      ...
      $title = apply_filters(
        'acf/fields/flexible_content/layout_title/name=' . $field['_name'],
        $title,
        $field,
        $layout,
        $i
      ...
      $title =
        '<span class="acf-fc-layout-order">' . $order . '</span> ' . $title;
