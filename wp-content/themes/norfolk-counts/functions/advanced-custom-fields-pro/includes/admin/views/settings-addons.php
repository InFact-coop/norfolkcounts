     ...
     $addon = wp_parse_args($addon, array(
       ...
       "btn_color" => ""
				...
				<div class="acf-box add-on add-on-<?php echo $addon['slug']; ?>">
					...
					<div class="inner">
						<h3><a target="_blank" href="<?php echo $addon['url']; ?>"><?php echo $addon[
  'title'
]; ?></a></h3>
					...
					<div class="footer">
						<?php if (
        apply_filters("acf/is_add_on_active/slug={$addon['slug']}", false)
      ): ?>
							<a class="button" disabled="disabled"><?php _e("Installed", 'acf'); ?></a>
						<?php else: ?>
							<a class="button <?php echo $addon[
         'btn_color'
       ]; ?>" target="_blank" href="<?php echo $addon['url']; ?>" ><?php _e(
  $addon['btn']
); ?></a>
						<?php endif; ?>
