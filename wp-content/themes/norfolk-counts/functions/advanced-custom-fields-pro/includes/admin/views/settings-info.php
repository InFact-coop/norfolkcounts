<div class="wrap about-wrap acf-wrap">
   ...
   __(
     "Thank you for updating! ACF %s is bigger and better than ever before. We hope you like it.",
     'acf'
   ),
   $version
 ); ?></div>
	...
	<h2 class="nav-tab-wrapper">
     ...
     $active == $tab_slug
   ): ?> nav-tab-active<?php endif; ?>" href="<?php echo admin_url(
  "edit.php?post_type=acf-field-group&page=acf-settings-info&tab={$tab_slug}"
); ?>"><?php echo $tab_title; ?></a>
		<?php endforeach; ?>
	</h2>
	
<?php if ($active == 'new'): ?>
	
	<h2 class="about-headline-callout"><?php _e(
   "A smoother custom field experience",
   'acf'
 ); ?></h2>
	...
	<div class="feature-section acf-three-col">
		<div>
			...
			<p><?php _e(
     "Including the popular Select2 library has improved both usability and speed across a number of field types including post object, page link, taxonomy and select.",
     'acf'
   ); ?></p>
		</div>
		<div>
			...
			<p><?php _e(
     "Many fields have undergone a visual refresh to make ACF look better than ever! Noticeable changes are seen on the gallery, relationship and oEmbed (new) fields!",
     'acf'
   ); ?></p>
		</div>
		<div>
			...
			<p><?php _e(
     "Redesigning the data architecture has allowed sub fields to live independently from their parents. This allows you to drag and drop fields in and out of parent fields!",
     'acf'
   ); ?></p>
		</div>
	</div>
   ...
   'acf'
	...
	<div class="feature-section acf-three-col">
     ...
     'acf'
			...
			<p><?php printf(
     __(
       'All 4 premium add-ons have been combined into a new <a href="%s">Pro version of ACF</a>. With both personal and developer licenses available, premium functionality is more affordable and accessible than ever before!',
       'acf'
     ),
     esc_url('https://www.advancedcustomfields.com/pro')
		...
		<div>
			...
			<p><?php _e(
     "ACF PRO contains powerful features such as repeatable data, flexible content layouts, a beautiful gallery field and the ability to create extra admin options pages!",
     'acf'
   ); ?></p>
			<p><?php printf(
     __('Read more about <a href="%s">ACF PRO features</a>.', 'acf'),
     esc_url('https://www.advancedcustomfields.com/pro')
   ); ?></p>
		</div>
		...
		<div>
			...
			<p><?php printf(
     __(
       'To help make upgrading easy, <a href="%s">login to your store account</a> and claim a free copy of ACF PRO!',
       'acf'
     ),
     esc_url('https://www.advancedcustomfields.com/my-account/')
   ); ?></p>
			<p><?php printf(
     __(
       'We also wrote an <a href="%s">upgrade guide</a> to answer any questions, but if you do have one, please contact our support team via the <a href="%s">help desk</a>',
       'acf'
     ),
     esc_url(
       'https://www.advancedcustomfields.com/resources/updates/upgrading-v4-v5/'
     ),
     esc_url('https://support.advancedcustomfields.com')
   ); ?>
...
<?php elseif ($active == 'changelog'): ?>
   ...
   __("We think you'll love the changes in %s.", 'acf'),
   $version
 ); ?></p>
 ...
 ?>
  ...
  if (
    isset($item[1])
  ): ?><a href="http<?php echo $item[1]; ?>" target="_blank">[...]</a><?php endif;
  ?></li>
	<?php
 endforeach; ?>
	</ul>
