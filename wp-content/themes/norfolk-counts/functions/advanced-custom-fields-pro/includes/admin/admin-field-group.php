...
if (!class_exists('acf_admin_field_group')):
  class acf_admin_field_group
  {
    ...
    function __construct()
    {
      ...
      // ajax
      add_action('wp_ajax_acf/field_group/render_field_settings', array(
        $this,
        'ajax_render_field_settings'
      ));
      add_action('wp_ajax_acf/field_group/render_location_rule', array(
        $this,
        'ajax_render_location_rule'
      ));
      add_action('wp_ajax_acf/field_group/move_field', array(
        $this,
        'ajax_move_field'
      ));
    ...
    function admin_head()
    {
      ...
      add_action(
        ...
        array($this, 'post_submitbox_misc_actions'),
        10,
        0
      );
      add_action(
...
<?php
    ...
    function ajax_move_field()
    {
      ...
      if ($args['field_id'] && $args['field_group_id']) {
        ...
        $a =
          '<a href="' .
          admin_url("post.php?post={$field_group['ID']}&action=edit") .
          '" target="_blank">' .
          $field_group['title'] .
          '</a>';
        ...
        echo '<p>' .
          sprintf(
            __('The %s field can now be found in the %s field group', 'acf'),
            $field['label'],
            $a
          ) .
          '</p>';
