...
function acf_update_500_field_group($ofg)
{
  ...
  if ($rows) {
    ...
    foreach ($rows as $row) {
      ...
      if (
        acf_has_done(
          'update_500_field_group_' . $ofg->ID . '_' . $row['meta_key']
        )
      ) {
        continue;
      }
...
{
  //acf_log('acf_update_550_termmeta');
...
{
  //acf_log('acf_update_550_taxonomy', $taxonomy);
  ...
  foreach ($rows as $row) {
    ...
    if (!preg_match($regexp, $row['option_name'], $matches)) {
      continue;
    }
