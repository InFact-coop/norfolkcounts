  ...
  class acf_admin_field_groups
  {
    ...
    function current_screen()
    {
      ...
      add_filter(
        ...
        array($this, 'field_group_columns'),
        10,
        1
      );
      add_action(
        'manage_acf-field-group_posts_custom_column',
        array($this, 'field_group_columns_html'),
        10,
        2
      );
    }
    ...
    function check_duplicate()
    {
      ...
      if ($ids = acf_maybe_get_GET('acfduplicatecomplete')) {
        ...
        if ($total == 1) {
          acf_add_admin_notice(
            sprintf(
              __('Field group duplicated. %s', 'acf'),
              '<a href="' .
      ...
      if (!empty($ids)) {
        ...
        wp_redirect(
          admin_url(
            $this->url . '&acfduplicatecomplete=' . implode(',', $new_ids)
          )
        );
        exit();
      }
    ...
    function check_sync()
    {
      ...
      if ($ids = acf_maybe_get_GET('acfsynccomplete')) {
        ...
        if ($total == 1) {
          acf_add_admin_notice(
            sprintf(
              __('Field group synchronised. %s', 'acf'),
              '<a href="' .
                get_edit_post_link($ids[0]) .
                '">' .
                get_the_title($ids[0]) .
      ...
      foreach ($groups as $group) {
        ...
        if ($local !== 'json' || $private) {
          ...
          $modified &&
          $modified > get_post_modified_time('U', true, $group['ID'], true)
        ) {
          $this->sync[$group['key']] = $group;
        }
      }
      ...
      if (!empty($sync_keys)) {
        ...
        // redirect
        wp_redirect(
          admin_url($this->url . '&acfsynccomplete=' . implode(',', $new_ids))
        );
        exit();
      }

      // filters
      add_filter('views_edit-acf-field-group', array(
        $this,
        'list_table_views'
      ));
    }
    ...
    function list_table_views($views)
    {
      ...
      $views['json'] =
        ...
        $class .
        ' href="' .
        admin_url($this->url . '&post_status=sync') .
        '">' .
        __('Sync available', 'acf') .
        ' <span class="count">(' .
        $total .
        ')</span></a>';
    ...
    function field_group_columns($columns)
    {
      return array(
        ...
        'acf-fg-status' =>
          '<i class="acf-icon -dot-3 small acf-js-tooltip" title="' .
          esc_attr__('Status', 'acf') .
          '"></i>',
    ...
    function render_column($column, $field_group)
    {
      ...
      if ($column == 'acf-fg-description') {
        ...
        if (isset($this->sync[$field_group['key']])) {
          echo '<i class="acf-icon -sync grey small acf-js-tooltip" title="' .
            esc_attr__('Sync available', 'acf') .
            '"></i> ';
        }
        ...
        if ($field_group['active']) {
          ...
          echo '<i class="acf-icon -minus yellow small acf-js-tooltip" title="' .
            esc_attr__('Inactive', 'acf') .
            '"></i> ';
        }
...
<div class="acf-column-2">
	<div class="acf-box">
     ...
     'acf'
			...
			<p><?php
   $acf_changelog = admin_url(
     'edit.php?post_type=acf-field-group&page=acf-settings-info&tab=changelog'
   );
   $acf_version = acf_get_setting('version');
   printf(
     __('See what\'s new in <a href="%s">version %s</a>.', 'acf'),
     esc_url($acf_changelog),
     $acf_version
   );
   ?></p>
      ...
      $url_home
    ); ?>" target="_blank"><?php echo $icon; ?> <?php _e(
   "Website",
   'acf'
 ); ?></a></li>
				<li><a href="<?php echo esc_url(
   ...
   'acf'
		...
		<div class="footer">
			<p><?php printf(
     __('Thank you for creating with <a href="%s">ACF</a>.', 'acf'),
     esc_url($url_home)
   ); ?></p>
		</div>
	</div>
...
(function($){
	...
	$('#the-list tr').each(function(){
		...
		if( id ) {
			...
			var url = '<?php echo esc_url(
     admin_url(
       $this->url .
         '&acfduplicate=__post_id__&_wpnonce=' .
         wp_create_nonce('bulk-posts')
     )
   ); ?>';
			var $span = $('<span class="acf-duplicate-field-group"><a title="<?php _e(
     'Duplicate this item',
     'acf'
   ); ?>" href="' + url.replace('__post_id__', post_id) + '"><?php _e(
  'Duplicate',
  'acf'
); ?></a> | </span>');
	...
	// modify bulk actions
	$('#bulk-action-selector-bottom option[value="edit"]').attr('value','acfduplicate').text('<?php _e(
   'Duplicate',
   'acf'
 ); ?>');
	...
	// search
	$('.subsubsub').append(' | <li><a href="#" class="acf-toggle-search"><?php _e(
   'Search',
   'acf'
 ); ?></a></li>');
...
<?php
    ...
    function sync_admin_footer()
    {
      // vars
      $i = -1;
      $columns = array('acf-fg-description', 'acf-fg-status', 'acf-fg-count');
...
<?php foreach ($this->sync as $field_group):
  ...
  $url = admin_url(
    $this->url . '&post_status=sync&acfsync=' . $key . '&_wpnonce=' . $nonce
  );
  ?>
	<tr <?php if ($i % 2 == 0): ?>class="alternate"<?php endif; ?>>
		<th class="check-column" scope="row">
			<label for="cb-select-<?php echo esc_attr(
     $key
   ); ?>" class="screen-reader-text"><?php echo esc_html(
  sprintf(__('Select %s', 'acf'), $title)
); ?></label>
			<input type="checkbox" value="<?php echo esc_attr(
     $key
   ); ?>" name="post[]" id="cb-select-<?php echo esc_attr($key); ?>">
		</th>
		<td class="post-title page-title column-title">
			<strong>
				<span class="row-title"><?php echo esc_html(
      $title
    ); ?></span><span class="acf-description"><?php echo esc_html(
  $key
); ?>.json</span>
			</strong>
			<div class="row-actions">
				<span class="import"><a title="<?php echo esc_attr(
      __('Synchronise field group', 'acf')
    ); ?>" href="<?php echo esc_url($url); ?>"><?php _e(
  'Sync',
  'acf'
); ?></a></span>
			</div>
		...
		<?php foreach ($columns as $column): ?>
			<td class="column-<?php echo esc_attr($column); ?>"><?php $this->render_column(
  $column,
  $field_group
); ?></td>
		<?php endforeach; ?>
...
<script type="text/html" id="tmpl-acf-bulk-actions">
	<?php
      // source: bulk_actions() wp-admin/includes/class-wp-list-table.php
      ?>
	<select name="action2" id="bulk-action-selector-bottom"></select>
	<?php submit_button(__('Apply'), 'action', '', false, array(
   'id' => "doaction2"
 )); ?>
</script>
<script type="text/javascript">
(function($){
	...
	// set only options
	$('#bulk-action-selector-bottom').html('<option value="-1"><?php _e(
   'Bulk Actions'
 ); ?></option><option value="acfsync"><?php _e('Sync', 'acf'); ?></option>');
