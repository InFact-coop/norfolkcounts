...
function acf_clone_field($field, $clone_field)
{
  ...
  $field = apply_filters(
    "acf/clone_field/type={$field['type']}",
    $field,
    $clone_field
  );
...
<<?php echo $el; ?> <?php acf_esc_attr_e($wrapper); ?>>
 ...
 ?></<?php echo $el2; ?>>
	...
	<<?php echo $el2; ?> class="acf-input">
		...
		<?php if ($instruction == 'field') {
    acf_render_field_instructions($field);
  } ?>
		<?php if (!empty($field['conditional_logic'])): ?>
		<script type="text/javascript">
			if( typeof acf !== 'undefined' ) { 
				acf.conditional_logic.add( '<?php echo esc_js(
      $field['key']
    ); ?>', <?php echo wp_json_encode($field['conditional_logic']); ?> ); 
			}
		</script>
		<?php endif; ?>
	</<?php echo $el2; ?>>
...
function acf_update_field($field = false, $specific = false)
{
  ...
  add_filter(
    'wp_unique_post_slug',
    'acf_update_field_wp_unique_post_slug',
    999,
    6
  );
...
function acf_update_field_wp_unique_post_slug(
  $slug,
  $post_ID,
  $post_status,
  $post_type,
...
function acf_prepare_field_for_export($field)
{
  ...
  $field = apply_filters(
    "acf/prepare_field_for_export/type={$field['type']}",
    $field
  );
...
function acf_prepare_field_for_import($field)
{
  ...
  $field = apply_filters(
    "acf/prepare_field_for_import/type={$field['type']}",
    $field
  );
...
function acf_get_sub_field($selector, $field)
{
  ...
  $sub_field = apply_filters(
    "acf/get_sub_field/type={$field['type']}",
    $sub_field,
    $selector,
    $field
  );
...
{
  // vars
  $keys = array('ID', 'name', 'key', '_name', '__name');

  // loop
  foreach ($keys as $k) {
    if (isset($field[$k]) && $field[$k] === $selector) {
      return true;
    }
  }
