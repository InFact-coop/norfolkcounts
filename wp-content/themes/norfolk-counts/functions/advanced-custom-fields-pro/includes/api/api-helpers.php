...
{
  return is_admin() && function_exists('get_user_locale')
    ? get_user_locale()
    : get_locale();
}
...
function acf_validate_attachment($attachment, $field, $context = 'prepare')
{
  ...
  if ($file['size']) {
    ...
    if ($min_size && $file['size'] < acf_get_filesize($min_size)) {
      ...
      $errors['max_size'] = sprintf(
        __('File size must must not exceed %s.', 'acf'),
  ...
  $errors = apply_filters(
    "acf/validate_attachment/name={$field['name']}",
    $errors,
    $file,
    $attachment,
    $field,
...
function acf_get_post_templates()
{
  ...
  // loop
  foreach ($post_types as $post_type) {
    $post_templates[$post_type] = wp_get_theme()->get_page_templates(
      null,
      $post_type
    );
  }
