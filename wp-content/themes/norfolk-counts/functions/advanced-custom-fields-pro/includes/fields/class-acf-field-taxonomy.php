...
if (!class_exists('acf_field_taxonomy')):
  class acf_field_taxonomy extends acf_field
  {
    ...
    function initialize()
    {
      ...
      // ajax
      add_action('wp_ajax_acf/fields/taxonomy/query', array(
        $this,
        'ajax_query'
      ));
      add_action('wp_ajax_nopriv_acf/fields/taxonomy/query', array(
        $this,
        'ajax_query'
      ));
      add_action('wp_ajax_acf/fields/taxonomy/add_term', array(
        $this,
        'ajax_add_term'
      ));
 ...
 if ($field['field_type'] == 'select') {
   ...
   $this->render_field_checkbox($field);
      ...
      if (!empty($field['value'])) {
        ...
        if (!empty($terms)) {
          foreach (array_keys($terms) as $i) {
            ...
            // append to choices
            $field['choices'][$term->term_id] = $this->get_term_title(
              $term,
              $field
            );
          }
    ...
    function render_field_checkbox($field)
    {
      ...
      $args = apply_filters(
        'acf/fields/taxonomy/wp_list_categories/name=' . $field['_name'],
        $args,
        $field
      );
      $args = apply_filters(
    ...
    function ajax_add_term()
    {
      ...
      if (!current_user_can($taxonomy_obj->cap->manage_terms)) {
        echo '<p><strong>' .
          __("Error.", 'acf') .
          '</strong> ' .
          sprintf(__('User unable to add new %s', 'acf'), $taxonomy_label) .
          '</p>';
        die();
      }
...
?><p class="acf-submit"><button class="acf-button button button-primary" type="submit"><?php _e(
  "Add",
  'acf'
); ?></button><i class="acf-spinner"></i><span></span></p></form><?php // die
die();
    }
  }
