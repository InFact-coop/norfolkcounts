...
if (!class_exists('acf_field_user')):
  class acf_field_user extends acf_field
  {
    ...
    function initialize()
    {
      ...
      add_action('wp_ajax_nopriv_acf/fields/user/query', array(
        $this,
        'ajax_query'
      ));
    }
    ...
    function get_ajax_query($options = array())
    {
      ...
      if ($is_search) {
        ...
        add_filter(
          'user_search_columns',
          array($this, 'user_search_columns'),
          10,
          3
        );
      }
      ...
      $args = apply_filters(
        "acf/fields/user/query/name={$field['_name']}",
        $args,
        $field,
        $options['post_id']
      );
      $args = apply_filters(
        "acf/fields/user/query/key={$field['key']}",
        $args,
        $field,
        $options['post_id']
      );
    ...
    function get_result($user, $field, $post_id = 0)
    {
      ...
      $result = apply_filters(
        "acf/fields/user/result/name={$field['_name']}",
        $result,
        $user,
        $field,
        $post_id
      ...
      $result = apply_filters(
        "acf/fields/user/result/key={$field['key']}",
        $result,
        $user,
        $field,
        $post_id
    ...
    function user_search_columns($columns, $search, $WP_User_Query)
    {
      ...
      $columns = apply_filters(
        "acf/fields/user/search_columns/name={$field['_name']}",
        $columns,
        $search,
        $WP_User_Query,
        $field
      ...
      $columns = apply_filters(
        "acf/fields/user/search_columns/key={$field['key']}",
        $columns,
        $search,
        $WP_User_Query,
        $field
